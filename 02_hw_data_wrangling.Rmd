...
## Answer 2
```{r}
party_imcumbent_promote <- fb_ads |>
  filter(office == "US House" | office == "US Senate") |>
  group_by(party, incumbency) |>
  summarize(promote_prop = mean(ad_tone_promote, na.rm = TRUE))
knitr::kable(party_imcumbent_promote, col.names = c("Party", "Incumbency", "Average Proportion of Ads that Mention Candidate Only"), digits = 3)
```

Incumbent democratic candidates sponsored the most promoting ads on average.
...
```
The histogram has a very steep drop-off/negative slope from 0 to 5 million of the x axis. Most of the House candidates had less than 10 million impressions on Facebook.
...
mapping = aes(
  x = impressions_millions , y = fct_reorder(cand_name_last , 
                                             impressions_millions))) +
    geom_col(mapping = aes(fill = party)) + scale_fill_manual(values = c(R = "red", D = "blue")) +
  labs(title = "Top 15 House Candidate Facebook Impressions, 2017-2018" , x = "Facebook Ad Impressions (in millions)")
